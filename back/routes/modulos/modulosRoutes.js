...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await modulosService.updateModulo(moduloId, nome, descricao);
    if (!result.updated) return res.status(404).json({ error: 'Módulo não encontrado' });

    res.json(result.modulo);
  } catch (err) {
    console.error(err);
